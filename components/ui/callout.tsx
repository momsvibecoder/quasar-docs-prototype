...
export function Callout({
  ...
  return (
    <div className={cn('q-callout overflow-hidden rounded-lg border text-sm leading-6', styles[type], className)} {...props}>
      <div className="q-callout-header flex items-center gap-2 p-6 text-base font-semibold leading-6 text-foreground">
        <Icon className="q-callout-icon h-4 w-4 shrink-0" aria-hidden="true" />
        <span>{label}</span>
      </div>
      <div className="q-callout-content border-t p-6 text-base leading-7 text-foreground [&>p:first-child]:mt-0 [&>p:last-child]:mb-0">
        {children}
      </div>
    </div>
  );
